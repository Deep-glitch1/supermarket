...
data_source = st.sidebar.radio(
    "Select Data Source",
    ("Upload File", "Use Flask API", "Use Sample Data")
)
...
# Handle file upload based on data source
...
elif data_source == "Use Flask API":
    st.sidebar.subheader("Connect to Flask API")
    
    # Button to load data from Flask API
    if st.sidebar.button("Load Data from API"):
        try:
            # Make API call to get data
            response = requests.get(f"{API_URL}/load-data")
            
            if response.status_code == 200:
                api_data = response.json().get('sample_data')
                st.session_state.data = load_data(api_data=api_data)
            else:
                st.sidebar.error(f"Error connecting to API: {response.status_code}")
        except Exception as e:
